...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      // 檢查庫存
        ...
        if (insufficientSizes.length > 0) {
            ...
            logger.warn("庫存不足");
            return sendError(res, response.insufficient_stock,`以下尺寸庫存不足: ${insufficientSizes.map((s) => `${s.size}(庫存${s.available}，需求${s.requested})`).join(", ")}`
            );
        }
        ...
        [JSON.stringify(updatedStock), newTotal, taipeiTime, product_id, specification]
        );
    }
     // 庫存紀錄
    await client.query(
    ...
    [ product_id,product_name, specification, JSON.stringify(quantities), taipeiTime,sale_id,'銷貨',total_quantity]
    );
